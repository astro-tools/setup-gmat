...
- A Linux or Windows runner. `ubuntu-latest` is the only configuration currently exercised in setup-gmat's CI. Other Linux runners with a recent glibc should work but are not covered by CI; cross-platform self-CI for `windows-latest` (and `macos-latest` when macOS support lands) is tracked under a future milestone.
