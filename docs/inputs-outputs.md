...
| ---------------- | -------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `version`        | no       | `R2026a` | GMAT release to install. Supported values: `R2022a`, `R2025a`, `R2026a` — R2023a and R2024a were never released by NASA, so they are not accepted (see [FAQ → What about R2023a and R2024a?](faq.md#what-about-r2023a-and-r2024a)). Any other value raises a validation error before the download starts. R2022a is **not** supported on `macos-latest` (its DMG is x86_64-only and Apple Silicon cannot dlopen it). Each release ships `gmatpy` bindings for a different Python range — see the [Python ABI table](https://github.com/astro-tools/setup-gmat#python-abi-per-gmat-release) in the README. |
...
| Name           | Description                                                                                   |
| -------------- | --------------------------------------------------------------------------------------------- |
| `gmat-root`    | Absolute path to the resolved GMAT install root. Equal to `$RUNNER_TEMP/gmat`.                |
